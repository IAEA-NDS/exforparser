...
def get_en_inc(pointer, locs, react_dict, data_dict_conv, data):
    ...
    if not locs["locs_en"] and not locs["locs_den"] and react_dict["process"] == "0,F":
        ...
        en_inc_frame = [ if_frame_in_cm( data_dict_conv["heads"][locs["locs_en"][0]] ) ] * len(data)
        ...
        for loc in locs["locs_en"]:
            if "-MIN" in data_dict_conv["heads"][loc]:
                en_inc_min =[
                    en / 1e6 if en is not None else None
                    for en in data_dict_conv["data"][loc]
                ]
                en_inc_frame = [ if_frame_in_cm( data_dict_conv["heads"][loc] )] * len(data)
            if "-MAX" in data_dict_conv["heads"][loc]:
                en_inc_max =[
                    en / 1e6 if en is not None else None
                    for en
                     in data_dict_conv["data"][loc]
                ]
                en_inc_frame = [ if_frame_in_cm( data_dict_conv["heads"][loc] )] * len(data)
...
def get_outgoing(pointer, locs, react_dict, data_dict_conv, data):
    ...
    if (len(locs["locs_e"]) == 1) or (
        ...
        e_out_frame = [ if_frame_in_cm( data_dict_conv["heads"][locs["locs_e"][0]] )] * len(data)
        e_out_min = [None] * len(data)
        e_out_max = [None] * len(data)
...
def process_general(entry_id, entry_json, data_dict_conv):
    ...
    ## -----------------------   Y (DATA)    --------------------- ##
    ## --------------------------------------------------------------------------------------- ##
    locs, data, ddata, data_unit_flag, ddata_unit_flag = get_y(entnum, subent, pointer, locs, data_dict_conv)
    ...
    ## -----------------------   Reaction residual    --------------------- ##
    ## --------------------------------------------------------------------------------------- ##
    locs, mass, charge, state, residual, residual_type = get_residual(locs, react_dict, data_dict_conv)

    ## --------------------------------------------------------------------------------------- ##
    ## -----------------------   Incident energy    --------------------- ##
    ## --------------------------------------------------------------------------------------- ##
    locs, en_inc, den_inc, en_inc_min, en_inc_max, en_inc_frame = get_en_inc(pointer, locs, react_dict, data_dict_conv, data)

    ## Special case for resonance parameters
    if not en_inc and any( res == react_dict["sf6"] for res in ("WID", "EN", "J", "L", "ARE", "WID/RED"  "WID/STR" )):
        en_pointer = None
        ...
        if en_pointer != 0:
            ...
            loc = [ l for l in range( len(data_dict_conv["heads"])) if data_dict_conv["heads"][l] == f"DATA      {en_pointer}" ]
            en_inc = [
                en / 1e6 if en is not None else None
                for en in data_dict_conv["data"][ loc[0] ]
            ]

    ## --------------------------------------------------------------------------------------- ##
    ##         Outgoing (E) or excitation energy (E-LVL)
    ## --------------------------------------------------------------------------------------- ##
    locs, e_out, de_out, e_out_min, e_out_max, e_out_frame, level_num, mt = get_outgoing(pointer, locs, react_dict, data_dict_conv, data)
...
def process_cross_section_case(df, entry_id, entry_json, react_dict):
    
    dir = get_dir_name(
        "exfortables_py", react_dict, level_num=None, subdir=None
    )
...
def process_partial_cross_section_case(df, entry_id, entry_json, react_dict):
...
def process_angler_distribution_section_case(df, entry_id, entry_json, react_dict):
    mf, mt = get_unique_mf_mt(df)
    dir = get_dir_name(
        "exfortables_py", react_dict, level_num=None, subdir=None
    )
...
def process_partial_angler_distribution_case(df, entry_id, entry_json, react_dict):
...
def process_energy_distribution_case(df, entry_id, entry_json, react_dict):
...
def process_neutron_observables_case(df, entry_id, entry_json, react_dict):

    mf, mt = get_unique_mf_mt(df)
    subdir = mt_nu_sf5[react_dict["sf5"]]["name"] if react_dict["sf5"] else ""
    dir = get_dir_name(
        "exfortables_py", react_dict, level_num=None, subdir=subdir
    )
...
def process_misc_neutron_observables_case(df, entry_id, entry_json, react_dict):
    mf, mt = get_unique_mf_mt(df)
    dir = get_dir_name(
        "exfortables_py", react_dict, level_num=None, subdir=None
    )
...
def process_kinetic_energy_case(df, entry_id, entry_json, react_dict):
    mf, mt = get_unique_mf_mt(df)
    dir = get_dir_name(
        "exfortables_py", react_dict, level_num=None, subdir=None
    )
...
def process_average_kinetic_energy_case(df, entry_id, entry_json, react_dict):
...
def process_fission_yield_case(df, entry_id, entry_json, react_dict):
