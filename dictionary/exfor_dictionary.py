...
def download_latest_dict():
    ...
    if local_max == latest:
        ...
        url = "".join([DICTIONARY_URL, "trans.", str(latest)])
        print(url)
        ...
        if r.status_code == 404:
            ...
            print("Downloaded", file, "from the IAEA:", DICTIONARY_URL)

    return latest


def dict_filename(latest):
    return "".join([DICTIONARY_PATH, "trans.", str(latest)])
...
def get_diction_difinition(latest) -> dict:
    ...
    for line in lines:
        if line.startswith("DICTION"):
            ...
            ## post json into mongodb
            if POST_DB:
                post_one_mongodb("dictiondef", dict2)
